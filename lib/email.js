...
export async function sendTicketEmail(name, email, ticketId) {
    ...
    try {
        ...
        await transporter.sendMail({
            ...
            to: `"${name}" <${email}>`,
            subject: `🎫 Your Iftar Gathering Ticket - ${ticketId}`,
            html: getTicketEmailHTML(name, ticketId),
            text: getTicketEmailPlainText(name, ticketId),
        });
...
<body style="margin: 0; padding: 0; background-color: #0A0F0D; font-family: Arial, sans-serif;">
    <div style="max-width: 500px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #132820; padding: 24px 32px; border-left: 1px solid rgba(212,175,55,0.2); border-right: 1px solid rgba(212,175,55,0.2);">
            ...
            <p style="font-size: 16px; color: #F5F5F0; margin: 0 0 20px;">${escapeHtml(name)}</p>
            ...
            <div style="background: rgba(0,0,0,0.3); border-radius: 12px; padding: 16px; text-align: center;">
                ...
                <p style="font-family: Courier New, monospace; font-size: 22px; font-weight: bold; color: #D4AF37; letter-spacing: 3px; margin: 0;">${escapeHtml(ticketId)}</p>
            </div>
        </div>
...
function getTicketEmailPlainText(name, ticketId) {
    return `IFTAR GATHERING - OLDHAM BROTHERS COMMUNITY
=============================================
